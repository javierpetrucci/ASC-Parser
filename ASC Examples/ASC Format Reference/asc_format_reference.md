...
|-----------|-------------|
| `type` | Component identifier (`res`, `cap`, `TCLib\OA_param2`, …). The actual SVG file used is the base name of the type (e.g., `OA_param2.svg`). |
| `x`, `y` | Placement coordinate on the sheet |
| `orientation` | Rotation/mirror code (see below) |
...
> [!IMPORTANT]
> **Alignment Override for Inductors:** Specifically for `ind`, when the orientation is **R0, R180, M0, or M180**, the alignment for Window 0 and Window 3 must **always be `Right`**, regardless of what is specified explicitly.
...
> Wires are purely geometric — just a line segment from `(x1,y1)` to `(x2,y2)`.
...
To determine orientation/alignment, find a **WIRE** that has one endpoint matching the FLAG coordinate `(x, y)`. The wire's *other* endpoint determines the incoming direction:
...
Draws a rectangle defined by opposite corners `(x1,y1)` and `(x2,y2)`.
...
Draws a circle (or ellipse) inscribed in the bounding box defined by `(x1,y1)` and `(x2,y2)`. 
...
Draws an arc from an ellipse defined by bounding box `(x1,y1)` and `(x2,y2)`.
...
Resistor at `(0,0)`, no rotation. InstName `R1` at offset `(36,40)` left-aligned. Value `1000` at offset `(36,76)` left-aligned.
